...
export const SubmissionModal: React.FC<SubmissionModalProps> = ({ test, user, initialImages = [], onClose, onSuccess }) => {
  ...
  const rotateImageInCanvas = async (imageSrc: string, degree: number): Promise<string> => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. Upload individual images to storage
      const pageIds: string[] = [];
      for (let i = 0; i < images.length; i++) {
        // If there's rotation, we should bake it into the image before upload
        let srcToUpload = images[i].src;
        if (images[i].rotation !== 0) {
          srcToUpload = await rotateImageInCanvas(images[i].src, images[i].rotation);
        }

        const response = await fetch(srcToUpload);
        const blob = await response.blob();
        const fileName = `Page_${i + 1}_${test.title}_${user.email}_${Date.now()}.jpg`;
        const fileId = await uploadFile(blob, fileName, 'developer@examfriendly.in', test.title, 'image/jpeg');
        pageIds.push(fileId);
      }
